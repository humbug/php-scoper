 ...
 * ...
 */
class StringMatchesFormatDescription extends \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Constraint\RegularExpression
{
    /**
     * @var string
     */
    ...
    protected function additionalFailureDescription($other) : string
    {
        ...
        $this->string = \implode("\n", $from);
        $other = \implode("\n", $to);
        $differ = new \_PhpScoper5b2c11ee6df50\SebastianBergmann\Diff\Differ("--- Expected\n+++ Actual\n");
        return $differ->diff($this->string, $other);
    }
    private function createPatternFromFormat(string $string) : string
    {
        $string = \preg_replace(['/(?<!%)%e/', '/(?<!%)%s/', '/(?<!%)%S/', '/(?<!%)%a/', '/(?<!%)%A/', '/(?<!%)%w/', '/(?<!%)%i/', '/(?<!%)%d/', '/(?<!%)%x/', '/(?<!%)%f/', '/(?<!%)%c/'], [\str_replace('\\', '\\\\', '\\' . \DIRECTORY_SEPARATOR), '[^\\r\\n]+', '[^\\r\\n]*', '.+', '.*', 's*', '[+-]?\\d+', 'd+', '[0-9a-fA-F]+', '[+-]?\\.?\\d+\\.?\\d*(?:[Ee][+-]?\\d+)?', '.'], \preg_quote($string, '/'));
