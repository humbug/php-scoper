    ...
    private static function parseShortOption(string $arg, string $short_options, array &$opts, array &$args) : void
    {
        ...
        for ($i = 0; $i < $argLen; $i++) {
            ...
            if ($arg[$i] === ':' || ($spec = \strstr($short_options, $opt)) === \false) {
                throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Exception("unrecognized option -- {$opt}");
            }
            if (\strlen($spec) > 1 && $spec[1] === ':') {
                ...
                if (!(\strlen($spec) > 2 && $spec[2] === ':')) {
                    ...
                    if (\false === ($opt_arg = \current($args))) {
                        throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Exception("option requires an argument -- {$opt}");
                    }
                    \next($args);
                }
            }
    ...
    private static function parseLongOption(string $arg, array $long_options, array &$opts, array &$args) : void
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ($opt_rest !== '' && $i + 1 < $count && $opt[0] !== '=' && \strpos($long_options[$i + 1], $opt) === 0) {
                throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Exception("option --{$opt} is ambiguous");
            }
            if (\substr($long_opt, -1) === '=') {
                ...
                if (\substr($long_opt, -2) !== '==' && !\strlen($opt_arg)) {
                    /* @noinspection ComparisonOperandsOrderInspection */
                    if (\false === ($opt_arg = \current($args))) {
                        throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Exception("option --{$opt} requires an argument");
                    }
                    \next($args);
                }
            } elseif ($opt_arg) {
                throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Exception("option --{$opt} doesn't allow an argument");
            }
