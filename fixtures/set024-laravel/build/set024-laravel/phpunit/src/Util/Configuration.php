...
final class Configuration
{
    ...
    public static function getInstance(string $filename) : self
    {
        ...
        if ($realPath === \false) {
            throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Exception(\sprintf('Could not read "%s".', $filename));
        }
        /** @var string $realPath */
    ...
    private function __construct(string $filename)
    {
        ...
        $this->document = \_PhpScoper5b2c11ee6df50\PHPUnit\Util\Xml::loadFile($filename, \false, \true, \true);
    ...
    public function getExtensionConfiguration() : array
    {
        ...
        foreach ($this->xpath->query('extensions/extension') as $extension) {
            ...
            if ($extension->getAttribute('file')) {
                $file = $this->toAbsolutePath((string) $extension->getAttribute('file'), \true);
            }
    ...
    public function getFilterConfiguration() : array
    {
        ...
        return ['whitelist' => ['addUncoveredFilesFromWhitelist' => $addUncoveredFilesFromWhitelist, 'processUncoveredFilesFromWhitelist' => $processUncoveredFilesFromWhitelist, 'include' => ['directory' => $includeDirectory, 'file' => $includeFile], 'exclude' => ['directory' => $excludeDirectory, 'file' => $excludeFile]]];
    }
    /**
    ...
    public function getPHPUnitConfiguration() : array
    {
        ...
        if ($root->hasAttribute('colors')) {
            ...
            if ($this->getBoolean($root->getAttribute('colors'), \false)) {
                $result['colors'] = \_PhpScoper5b2c11ee6df50\PHPUnit\TextUI\ResultPrinter::COLOR_AUTO;
            } else {
                $result['colors'] = \_PhpScoper5b2c11ee6df50\PHPUnit\TextUI\ResultPrinter::COLOR_NEVER;
            }
        }
        /*
        ...
        if ($root->hasAttribute('convertDeprecationsToExceptions')) {
            $result['convertDeprecationsToExceptions'] = $this->getBoolean((string) $root->getAttribute('convertDeprecationsToExceptions'), \true);
        }
        ...
        if ($root->hasAttribute('beStrictAboutTestsThatDoNotTestAnything')) {
            $result['reportUselessTests'] = $this->getBoolean((string) $root->getAttribute('beStrictAboutTestsThatDoNotTestAnything'), \true);
        }
        ...
        if ($root->hasAttribute('executionOrder')) {
            switch ((string) $root->getAttribute('executionOrder')) {
                case 'random':
                    $result['executionOrder'] = \_PhpScoper5b2c11ee6df50\PHPUnit\Runner\TestSuiteSorter::ORDER_RANDOMIZED;
                    break;
                case 'reverse':
                    $result['executionOrder'] = \_PhpScoper5b2c11ee6df50\PHPUnit\Runner\TestSuiteSorter::ORDER_REVERSED;
                    break;
                default:
                    $result['executionOrder'] = \_PhpScoper5b2c11ee6df50\PHPUnit\Runner\TestSuiteSorter::ORDER_DEFAULT;
            }
        }
        if ($root->hasAttribute('resolveDependencies')) {
            $result['resolveDependencies'] = $this->getBoolean((string) $root->getAttribute('resolveDependencies'), \false);
        }
        return $result;
    }
    /**
     ...
     * @throws Exception
     */
    public function getTestSuiteConfiguration(string $testSuiteFilter = '') : \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestSuite
    {
        ...
        $suite = new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestSuite();
        ...
        foreach ($nodes as $node) {
            ...
            foreach ($node->childNodes as $argument) {
                ...
                if ($argument->tagName === 'file' || $argument->tagName === 'directory') {
                    ...
                    $arguments[] = \_PhpScoper5b2c11ee6df50\PHPUnit\Util\Xml::xmlToVariable($argument);
                }
            }
        }
        return $arguments;
     ...
     */
    private function getTestSuite(\DOMElement $testSuiteNode, string $testSuiteFilter = '') : \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestSuite
    {
        if ($testSuiteNode->hasAttribute('name')) {
            $suite = new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestSuite((string) $testSuiteNode->getAttribute('name'));
        } else {
            $suite = new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestSuite();
        }
        $exclude = [];
        ...
        $fileIteratorFacade = new \_PhpScoper5b2c11ee6df50\SebastianBergmann\FileIterator\Facade();
