...
use _PhpScoper5b2c11ee6df50\PharIo\Version\VersionConstraintParser;
use _PhpScoper5b2c11ee6df50\PHPUnit\Framework\CodeCoverageException;
use _PhpScoper5b2c11ee6df50\PHPUnit\Framework\Exception;
use _PhpScoper5b2c11ee6df50\PHPUnit\Framework\InvalidCoversTargetException;
...
use _PhpScoper5b2c11ee6df50\SebastianBergmann\Environment\OperatingSystem;
use Traversable;
final class Test
{
    /**
    ...
    public static function describe(\_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Test $test) : array
    {
    ...
    public static function describeAsString(\_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Test $test) : string
    {
        if ($test instanceof \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\SelfDescribing) {
            return $test->toString();
        }
    ...
    public static function getRequirements(string $className, string $methodName) : array
    {
        ...
        if ($count = \preg_match_all(self::REGEX_REQUIRES_VERSION_CONSTRAINT, $docComment, $matches)) {
            foreach (\range(0, $count - 1) as $i) {
                ...
                try {
                    $versionConstraintParser = new \_PhpScoper5b2c11ee6df50\PharIo\Version\VersionConstraintParser();
                    $requires[$matches['name'][$i] . '_constraint'] = ['constraint' => $versionConstraintParser->parse(\trim($matches['constraint'][$i]))];
                } catch (\_PhpScoper5b2c11ee6df50\PharIo\Version\Exception $e) {
                    throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Warning($e->getMessage(), $e->getCode(), $e);
                }
            }
        }
    ...
    public static function getMissingRequirements(string $className, string $methodName) : array
    {
        ...
        if (!empty($required['PHP'])) {
            ...
            $version = new \_PhpScoper5b2c11ee6df50\PharIo\Version\Version(self::sanitizeVersionNumber(\PHP_VERSION));
        ...
        if (!empty($required['PHPUnit'])) {
            $phpunitVersion = \_PhpScoper5b2c11ee6df50\PHPUnit\Runner\Version::id();
            ...
            $phpunitVersion = new \_PhpScoper5b2c11ee6df50\PharIo\Version\Version(self::sanitizeVersionNumber(\_PhpScoper5b2c11ee6df50\PHPUnit\Runner\Version::id()));
        ...
        if (!empty($required['OSFAMILY']) && $required['OSFAMILY'] !== (new \_PhpScoper5b2c11ee6df50\SebastianBergmann\Environment\OperatingSystem())->getFamily()) {
    ...
    public static function getProvidedData(string $className, string $methodName) : ?array
    {
        ...
        if (\is_array($data) && empty($data)) {
            throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\SkippedTestError();
        }
        if ($data !== null) {
            foreach ($data as $key => $value) {
                if (!\is_array($value)) {
                    throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Exception(\sprintf('Data set %s is invalid.', \is_int($key) ? '#' . $key : '"' . $key . '"'));
                }
            }
        }
        return $data;
        ...
        if (\preg_match(self::REGEX_TEST_WITH, $docComment, $matches, \PREG_OFFSET_CAPTURE)) {
            ...
            foreach (\explode("\n", $annotationContent) as $candidateRow) {
                ...
                if (\json_last_error() !== \JSON_ERROR_NONE) {
                    throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Exception('The data set for the @testWith annotation cannot be parsed: ' . \json_last_error_msg());
                }
                $data[] = $dataSet;
            }
            if (!$data) {
                throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Exception('The data set for the @testWith annotation cannot be parsed.');
            }
            return $data;
        }
        return null;
    ...
    public static function parseTestMethodAnnotations(string $className, ?string $methodName = '') : array
    {
        ...
        return ['class' => self::$annotationCache[$className], 'method' => $methodName !== null ? self::$annotationCache[$cacheKey] : []];
    }
    ...
    private static function getLinesToBeCoveredOrUsed(string $className, string $methodName, string $mode) : array
    {
        ...
        if (!empty($annotations['class'][$mode . 'DefaultClass'])) {
            if (\count($annotations['class'][$mode . 'DefaultClass']) > 1) {
                throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\CodeCoverageException(\sprintf('More than one @%sClass annotation in class or interface "%s".', $mode, $className));
            }
            ...
            if (isset($methodName[0]) && $methodName[0] === '<') {
                ...
                foreach ($classes as $className) {
                    if (!\class_exists($className) && !\interface_exists($className) && !\trait_exists($className)) {
                        throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\InvalidCoversTargetException(\sprintf('Trying to @cover or @use not existing class or ' . 'interface "%s".', $className));
                    }
                    $class = new \ReflectionClass($className);
                ...
                foreach ($classes as $className) {
                    if ($className === '' && \function_exists($methodName)) {
                        ...
                        if (!((\class_exists($className) || \interface_exists($className) || \trait_exists($className)) && \method_exists($className, $methodName))) {
                            throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\InvalidCoversTargetException(\sprintf('Trying to @cover or @use not existing method "%s::%s".', $className, $methodName));
                        }
