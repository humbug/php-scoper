...
use _PhpScoper5b2c11ee6df50\PHPUnit\Framework\IncompleteTestError;
use _PhpScoper5b2c11ee6df50\PHPUnit\Framework\SelfDescribing;
use _PhpScoper5b2c11ee6df50\PHPUnit\Framework\SkippedTestError;
 ...
 */
class PhptTestCase implements \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Test, \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\SelfDescribing
{
    /**
     * @var string[]
     */
    private const SETTINGS = ['allow_url_fopen=1', 'auto_append_file=', 'auto_prepend_file=', 'disable_functions=', 'display_errors=1', 'docref_root=', 'docref_ext=.html', 'error_append_string=', 'error_prepend_string=', 'error_reporting=-1', 'html_errors=0', 'log_errors=0', 'magic_quotes_runtime=0', 'output_handler=', 'open_basedir=', 'output_buffering=Off', 'report_memleaks=0', 'report_zend_debug=0', 'safe_mode=0', 'xdebug.default_enable=0'];
    /**
     * @var string
     */
    private $filename;
     ...
     * @throws Exception
     */
    public function __construct(string $filename, \_PhpScoper5b2c11ee6df50\PHPUnit\Util\PHP\AbstractPhpProcess $phpUtil = null)
    {
        if (!\is_file($filename)) {
            throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Runner\Exception(\sprintf('File "%s" does not exist.', $filename));
        }
        $this->filename = $filename;
        $this->phpUtil = $phpUtil ?: \_PhpScoper5b2c11ee6df50\PHPUnit\Util\PHP\AbstractPhpProcess::factory();
    }
    /**
    ...
    public function run(\_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestResult $result = null) : \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestResult
    {
        ...
        if ($result === null) {
            $result = new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestResult();
        }
        $xfail = \false;
        ...
        if ($result->getCollectCodeCoverageInformation() && ($coverage = $this->cleanupForCoverage())) {
            $result->getCodeCoverage()->append($coverage, $this, \true, [], [], \true);
        }
        try {
            ...
            $failure = $e;
            if ($xfail !== \false) {
                $failure = new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\IncompleteTestError($xfail, 0, $e);
            }
        ...
        if ($result->allCompletelyImplemented() && $xfail !== \false) {
            $result->addFailure($this, new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\IncompleteTestError('XFAIL section but test passes'), $time);
        }
    ...
    private function assertPhptExpectation(array $sections, string $output) : void
    {
        ...
        if (!isset($assertion)) {
            throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Runner\Exception('No PHPT assertion found');
        }
        if (!isset($expected)) {
            throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Runner\Exception('No PHPT expectation found');
        }
        \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Assert::$assertion($expected, $actual);
    }
    /**
     * @throws \SebastianBergmann\RecursionContext\InvalidArgumentException
     */
    private function runSkip(array &$sections, \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestResult $result, array $settings) : bool
    {
        ...
        if (!\strncasecmp('skip', \ltrim($jobResult['stdout']), 4)) {
            ...
            $result->addFailure($this, new \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\SkippedTestError($message), 0);
    ...
    private function parse() : array
    {
        ...
        foreach (\file($this->filename) as $line) {
            ...
            if (empty($section)) {
                throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Runner\Exception('Invalid PHPT file');
            }
        ...
        foreach ($unsupportedSections as $section) {
            if (isset($sections[$section])) {
                throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Runner\Exception('PHPUnit does not support this PHPT file');
            }
        }
        return $sections;
    }
        ...
        foreach ($allowSections as $section) {
            if (isset($sections[$section . '_EXTERNAL'])) {
                ...
                if (!\is_file($testDirectory . $externalFilename) || !\is_readable($testDirectory . $externalFilename)) {
                    throw new \_PhpScoper5b2c11ee6df50\PHPUnit\Runner\Exception(\sprintf('Could not load --%s-- %s for PHPT file', $section . '_EXTERNAL', $testDirectory . $externalFilename));
                }
    ...
    private function renderForCoverage(array &$settings) : void
    {
        ...
        $template = new \_PhpScoper5b2c11ee6df50\Text_Template(__DIR__ . '/../Util/PHP/Template/PhptTestCase.tpl');
        $composerAutoload = '\'\'';
        ...
        $template->setVar(['composerAutoload' => $composerAutoload, 'phar' => $phar, 'globals' => $globals, 'job' => $files['job'], 'coverageFile' => $files['coverage'], 'autoPrependFile' => \var_export(!empty($settings['auto_prepend_file']) ? $settings['auto_prepend_file'] : \false, \true)]);
