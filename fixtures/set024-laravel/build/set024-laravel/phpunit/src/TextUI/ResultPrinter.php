...
use _PhpScoper5b2c11ee6df50\SebastianBergmann\Environment\Console;
use _PhpScoper5b2c11ee6df50\SebastianBergmann\Timer\Timer;
/**
 * Prints the result of a TextUI TestRunner run.
 */
class ResultPrinter extends \_PhpScoper5b2c11ee6df50\PHPUnit\Util\Printer implements \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestListener
{
    ...
    public function __construct($out = null, bool $verbose = \false, $colors = self::COLOR_DEFAULT, bool $debug = \false, $numberOfColumns = 80, bool $reverse = \false)
    {
        ...
        if (!\in_array($colors, self::AVAILABLE_COLORS, \true)) {
            throw \_PhpScoper5b2c11ee6df50\PHPUnit\Util\InvalidArgumentHelper::factory(3, \vsprintf('value from "%s", "%s" or "%s"', self::AVAILABLE_COLORS));
        }
        if (!\is_int($numberOfColumns) && $numberOfColumns !== 'max') {
            throw \_PhpScoper5b2c11ee6df50\PHPUnit\Util\InvalidArgumentHelper::factory(5, 'integer or "max"');
        }
        $console = new \_PhpScoper5b2c11ee6df50\SebastianBergmann\Environment\Console();
    ...
    {
        if ($this->debug) {
            $this->write(\sprintf("Test '%s' started\n", \_PhpScoper5b2c11ee6df50\PHPUnit\Util\Test::describeAsString($test)));
        }
    }
    /**
     * A test ended.
    ...
    public function endTest(\_PhpScoper5b2c11ee6df50\PHPUnit\Framework\Test $test, float $time) : void
    {
        ...
        if ($test instanceof \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestCase && !$test->hasExpectationOnOutput()) {
    ...
    protected function printDefect(\_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestFailure $defect, int $count) : void
    {
        $this->printDefectHeader($defect, $count);
        $this->printDefectTrace($defect);
    }
    protected function printDefectHeader(\_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestFailure $defect, int $count) : void
    {
        $this->write(\sprintf("\n%d) %s\n", $count, $defect->getTestName()));
    }
    protected function printDefectTrace(\_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestFailure $defect) : void
    {
    ...
    protected function printErrors(\_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestResult $result) : void
    {
        $this->printDefects($result->errors(), 'error');
    }
    protected function printFailures(\_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestResult $result) : void
    {
        $this->printDefects($result->failures(), 'failure');
    }
    protected function printWarnings(\_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestResult $result) : void
    {
        $this->printDefects($result->warnings(), 'warning');
    }
    protected function printIncompletes(\_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestResult $result) : void
    {
        $this->printDefects($result->notImplemented(), 'incomplete test');
    }
    protected function printRisky(\_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestResult $result) : void
    {
        $this->printDefects($result->risky(), 'risky test');
    }
    protected function printSkipped(\_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestResult $result) : void
    {
    ...
    protected function printFooter(\_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestResult $result) : void
    {
        if (\count($result) === 0) {
