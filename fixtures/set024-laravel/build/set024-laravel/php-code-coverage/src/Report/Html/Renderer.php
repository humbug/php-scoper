...
abstract class Renderer
{
    ...
    public function __construct(string $templatePath, string $generator, string $date, int $lowUpperBound, int $highLowerBound)
    {
        ...
        $this->version = \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Version::id();
    }
    protected function renderItemTemplate(\_PhpScoper5b2c11ee6df50\Text_Template $template, array $data) : string
    {
        $numSeparator = '&nbsp;/&nbsp;';
    ...
    protected function setCommonTemplateVariables(\_PhpScoper5b2c11ee6df50\Text_Template $template, \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Node\AbstractNode $node) : void
    {
    ...
    protected function getBreadcrumbs(\_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Node\AbstractNode $node) : string
    {
        $breadcrumbs = '';
        $path = $node->getPathAsArray();
        $pathToRoot = [];
        $max = \count($path);
        if ($node instanceof \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Node\File) {
            $max--;
        }
        for ($i = 0; $i < $max; $i++) {
    ...
    protected function getActiveBreadcrumb(\_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Node\AbstractNode $node) : string
    {
    ...
    protected function getInactiveBreadcrumb(\_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Node\AbstractNode $node, string $pathToRoot) : string
    {
        return \sprintf('        <li><a href="%sindex.html">%s</a></li>' . "\n", $pathToRoot, $node->getName());
    }
    protected function getPathToRoot(\_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Node\AbstractNode $node) : string
    {
        $id = $node->getId();
        $depth = \substr_count($id, '/');
        if ($id !== 'index' && $node instanceof \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Node\Directory) {
            $depth++;
        }
    ...
    protected function getCoverageBar(float $percent) : string
    {
        ...
        $template = new \_PhpScoper5b2c11ee6df50\Text_Template($this->templatePath . 'coverage_bar.html', '{{', '}}');
