 ...
 * Renders a file node.
 */
final class File extends \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Report\Html\Renderer
{
    /**
     * @var int
     */
     ...
     */
    public function render(\_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Node\File $node, string $file) : void
    {
        $template = new \_PhpScoper5b2c11ee6df50\Text_Template($this->templatePath . 'file.html', '{{', '}}');
        $template->setVar(['items' => $this->renderItems($node), 'lines' => $this->renderSource($node)]);
        $this->setCommonTemplateVariables($template, $node);
        $template->renderTo($file);
    }
    protected function renderItems(\_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Node\File $node) : string
    {
        $template = new \_PhpScoper5b2c11ee6df50\Text_Template($this->templatePath . 'file_item.html', '{{', '}}');
        $methodItemTemplate = new \_PhpScoper5b2c11ee6df50\Text_Template($this->templatePath . 'method_item.html', '{{', '}}');
    ...
    protected function renderTraitOrClassItems(array $items, \_PhpScoper5b2c11ee6df50\Text_Template $template, \_PhpScoper5b2c11ee6df50\Text_Template $methodItemTemplate) : string
    {
        $buffer = '';
        if (empty($items)) {
            return $buffer;
        ...
        foreach ($items as $name => $item) {
            ...
            if ($item['executableLines'] > 0) {
                ...
                $linesExecutedPercentAsString = \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Util::percent($item['executedLines'], $item['executableLines'], \true);
            } else {
                $numClasses = 'n/a';
                $numTestedClasses = 'n/a';
                $linesExecutedPercentAsString = 'n/a';
            }
            $buffer .= $this->renderItemTemplate($template, ['name' => $name, 'numClasses' => $numClasses, 'numTestedClasses' => $numTestedClasses, 'numMethods' => $numMethods, 'numTestedMethods' => $numTestedMethods, 'linesExecutedPercent' => \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Util::percent($item['executedLines'], $item['executableLines'], \false), 'linesExecutedPercentAsString' => $linesExecutedPercentAsString, 'numExecutedLines' => $item['executedLines'], 'numExecutableLines' => $item['executableLines'], 'testedMethodsPercent' => \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Util::percent($numTestedMethods, $numMethods), 'testedMethodsPercentAsString' => \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Util::percent($numTestedMethods, $numMethods, \true), 'testedClassesPercent' => \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Util::percent($numTestedMethods == $numMethods ? 1 : 0, 1), 'testedClassesPercentAsString' => \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Util::percent($numTestedMethods == $numMethods ? 1 : 0, 1, \true), 'crap' => $item['crap']]);
            foreach ($item['methods'] as $method) {
    ...
    protected function renderFunctionItems(array $functions, \_PhpScoper5b2c11ee6df50\Text_Template $template) : string
    {
        if (empty($functions)) {
            return '';
        }
    ...
    protected function renderFunctionOrMethodItem(\_PhpScoper5b2c11ee6df50\Text_Template $template, array $item, string $indent = '') : string
    {
        $numMethods = 0;
        $numTestedMethods = 0;
        if ($item['executableLines'] > 0) {
     ...
     * @return string
     */
    protected function renderSource(\_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Node\File $node) : string
    {
        ...
        foreach ($codeLines as $line) {
            ...
            if (\array_key_exists($i, $coverageData)) {
                ...
                if ($coverageData[$i] === null) {
                    ...
                    foreach ($coverageData[$i] as $test) {
                        ...
                        $popoverContent .= \sprintf('<li%s>%s</li>', $testCSS, \htmlspecialchars($test, $this->htmlSpecialCharsFlags));
                    }
                    $popoverContent .= '</ul>';
            ...
            if (!empty($popoverTitle)) {
                $popover = \sprintf(' data-title="%s" data-content="%s" data-placement="bottom" data-html="true"', $popoverTitle, \htmlspecialchars($popoverContent, $this->htmlSpecialCharsFlags));
            }
