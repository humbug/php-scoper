...
use _PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Driver\Driver;
use _PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Driver\PHPDBG;
use _PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Driver\Xdebug;
use _PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Node\Builder;
use _PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Node\Directory;
use _PhpScoper5b2c11ee6df50\SebastianBergmann\CodeUnitReverseLookup\Wizard;
...
final class CodeCoverage
{
     ...
     */
    public function __construct(\_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Driver\Driver $driver = null, \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Filter $filter = null)
    {
        if ($driver === null) {
        ...
        $this->driver = $driver;
        $this->filter = $filter;
        $this->wizard = new \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeUnitReverseLookup\Wizard();
    }
    /**
     * Returns the code coverage information as a graph of node objects.
     */
    public function getReport() : \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Node\Directory
    {
        if ($this->report === null) {
            $builder = new \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Node\Builder();
     ...
     */
    public function filter() : \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Filter
    {
        return $this->filter;
    }
    /**
     ...
     */
    public function stop(bool $append = \true, $linesToBeCovered = [], array $linesToBeUsed = [], bool $ignoreForceCoversAnnotation = \false) : array
    {
        if (!\is_array($linesToBeCovered) && $linesToBeCovered !== \false) {
            throw \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\InvalidArgumentException::create(2, 'array or false');
        }
     ...
     * @throws RuntimeException
     */
    public function append(array $data, $id = null, bool $append = \true, $linesToBeCovered = [], array $linesToBeUsed = [], bool $ignoreForceCoversAnnotation = \false) : void
    {
        if ($id === null) {
            $id = $this->currentId;
        }
        if ($id === null) {
            throw new \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\RuntimeException();
        }
        ...
        $size = 'unknown';
        $status = -1;
        if ($id instanceof \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestCase) {
            $_size = $id->getSize();
            if ($_size === \_PhpScoper5b2c11ee6df50\PHPUnit\Util\Test::SMALL) {
                $size = 'small';
            } elseif ($_size === \_PhpScoper5b2c11ee6df50\PHPUnit\Util\Test::MEDIUM) {
                $size = 'medium';
            } elseif ($_size === \_PhpScoper5b2c11ee6df50\PHPUnit\Util\Test::LARGE) {
                $size = 'large';
            }
            $status = $id->getStatus();
            $id = \get_class($id) . '::' . $id->getName();
        } elseif ($id instanceof \_PhpScoper5b2c11ee6df50\PHPUnit\Runner\PhptTestCase) {
            $size = 'large';
            $id = $id->getName();
        }
        ...
        foreach ($data as $file => $lines) {
            ...
            foreach ($lines as $k => $v) {
                if ($v === \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Driver\Driver::LINE_EXECUTED) {
    ...
    public function merge(self $that) : void
    {
        ...
        foreach ($that->data as $file => $lines) {
            ...
            $compareLineNumbers = \array_unique(\array_merge(\array_keys($this->data[$file]), \array_keys($that->data[$file])));
            foreach ($compareLineNumbers as $line) {
                ...
                if ($thatPriority > $thisPriority) {
                    ...
                    $this->data[$file][$line] = \array_unique(\array_merge($this->data[$file][$line], $that->data[$file][$line]));
                }
            }
        }
    ...
    private function applyCoversAnnotationFilter(array &$data, $linesToBeCovered, array $linesToBeUsed, bool $ignoreForceCoversAnnotation) : void
    {
        if ($linesToBeCovered === \false || $this->forceCoversAnnotation && empty($linesToBeCovered) && !$ignoreForceCoversAnnotation) {
            if ($this->checkForMissingCoversAnnotation) {
                throw new \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\MissingCoversAnnotationException();
            }
            $data = [];
            return;
        }
        ...
        if ($this->checkForUnintentionallyCoveredCode && (!$this->currentId instanceof \_PhpScoper5b2c11ee6df50\PHPUnit\Framework\TestCase || !$this->currentId->isMedium() && !$this->currentId->isLarge())) {
    ...
    private function addUncoveredFilesFromWhitelist() : void
    {
        ...
        foreach ($uncoveredFiles as $uncoveredFile) {
            ...
            for ($i = 1; $i <= $lines; $i++) {
                $data[$uncoveredFile][$i] = \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Driver\Driver::LINE_NOT_EXECUTED;
            }
        }
    ...
    private function getLinesToBeIgnored(string $fileName) : array
    {
        ...
        if ($this->cacheTokens) {
            $tokens = \_PhpScoper5b2c11ee6df50\PHP_Token_Stream_CachingFactory::get($fileName);
        } else {
            $tokens = new \_PhpScoper5b2c11ee6df50\PHP_Token_Stream($fileName);
        }
        ...
        foreach ($tokens->tokens() as $token) {
            switch (\get_class($token)) {
                case \_PhpScoper5b2c11ee6df50\PHP_Token_COMMENT::class:
                case \_PhpScoper5b2c11ee6df50\PHP_Token_DOC_COMMENT::class:
                    $_token = \trim($token);
                    ...
                    break;
                case \_PhpScoper5b2c11ee6df50\PHP_Token_INTERFACE::class:
                case \_PhpScoper5b2c11ee6df50\PHP_Token_TRAIT::class:
                case \_PhpScoper5b2c11ee6df50\PHP_Token_CLASS::class:
                case \_PhpScoper5b2c11ee6df50\PHP_Token_FUNCTION::class:
                ...
                case \_PhpScoper5b2c11ee6df50\PHP_Token_NAMESPACE::class:
                    $this->ignoredLines[$fileName][] = $token->getEndLine();
                // Intentional fallthrough
                case \_PhpScoper5b2c11ee6df50\PHP_Token_DECLARE::class:
                case \_PhpScoper5b2c11ee6df50\PHP_Token_OPEN_TAG::class:
                case \_PhpScoper5b2c11ee6df50\PHP_Token_CLOSE_TAG::class:
                case \_PhpScoper5b2c11ee6df50\PHP_Token_USE::class:
                    $this->ignoredLines[$fileName][] = $token->getLine();
                    break;
            }
            if ($ignore) {
    ...
    private function performUnintentionallyCoveredCodeCheck(array &$data, array $linesToBeCovered, array $linesToBeUsed) : void
    {
        ...
        if (!empty($unintentionallyCoveredUnits)) {
            throw new \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\UnintentionallyCoveredCodeException($unintentionallyCoveredUnits);
        }
    }
    /**
     * @throws CoveredCodeNotExecutedException
    ...
    private function performUnexecutedCoveredCodeCheck(array &$data, array $linesToBeCovered, array $linesToBeUsed) : void
    {
        ...
        if (!empty($message)) {
            throw new \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\CoveredCodeNotExecutedException($message);
        }
    }
     ...
     * @throws RuntimeException
     */
    private function selectDriver() : \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Driver\Driver
    {
        $runtime = new \_PhpScoper5b2c11ee6df50\SebastianBergmann\Environment\Runtime();
        if (!$runtime->canCollectCodeCoverage()) {
            throw new \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\RuntimeException('No code coverage driver available');
        }
        if ($runtime->isPHPDBG()) {
            return new \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Driver\PHPDBG();
        }
        if ($runtime->hasXdebug()) {
            return new \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Driver\Xdebug();
        }
        throw new \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\RuntimeException('No code coverage driver available');
    }
        ...
        if ($this->processUncoveredFilesFromWhitelist) {
            ...
            foreach ($coverage as $file => $fileCoverage) {
                ...
                foreach (\array_keys($fileCoverage) as $key) {
                    if ($fileCoverage[$key] === \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Driver\Driver::LINE_EXECUTED) {
                        $fileCoverage[$key] = \_PhpScoper5b2c11ee6df50\SebastianBergmann\CodeCoverage\Driver\Driver::LINE_NOT_EXECUTED;
                    }
                }
