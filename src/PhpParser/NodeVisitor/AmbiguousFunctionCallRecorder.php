...
/**
 * Records the functions calls which are ambiguous, i.e. it cannot be determined whether the called function is from
 * the current namespace or if it is a globally declared function.
 *
 * @private
 */
final readonly class AmbiguousFunctionCallRecorder extends NodeVisitorAbstract
{
    ...
    public function enterNode(Node $node): Node
    {
        ...
        if (!($node instanceof FullyQualified)
            && self::isAFunctionCall($node)
            && $this->enrichedReflector->isExposed
        ) {
            $this->symbolsRegistry->recordAmbiguousFunctionCall($node);
        }

        return $node;
    }

    private static function isAFunctionCall(Name $name): bool
    {
